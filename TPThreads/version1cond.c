...
// Paramètres de ma fonction threads. Pour transmettre une information à un thread, c'est ici !
struct paramsFonctionThread {
  int idThread;
  struct variablesCommunes* varCommunes;
};

// NbZone, verrou et cond n'ont pas besoin d'être des pointeurs
// En effet, on déclare une seule instance de la structure varCommunes dans le main et c'est à elle
// que l'on assigne un entier pour nbZone, un verrou à verrou et un condition à cond
...
void * traitement (void * p) {
  ...
  for(int i = 0; i <= varCommunes->nbZone; i++){
      ...
      if( args -> idThread != 0){  // le premier traitement n'attend personne
        ...
        while(varCommunes->di[args->idThread-1]<=i)
        {
          printf("[%i] Mise en attente : j'attends pour traiter la zone : %i \n",args->idThread, i);
          pthread_cond_wait(&(varCommunes->cond), &(varCommunes->verrou));
        }
      }
      ...
      pthread_cond_broadcast(&(varCommunes->cond));
  }
...
int main(int argc, char * argv[]){
  ...
  // cond 
  // Même remarque qu'au dessus, on peut initialiser de plusieurs façons
  pthread_cond_init(&(varCommunes.cond),NULL);
  ...
  srand(atoi(argv[1]));  // initialisation de rand pour la simulation de longs calculs (avec calcul())
  ...
  pthread_cond_destroy(&(varCommunes.cond));
